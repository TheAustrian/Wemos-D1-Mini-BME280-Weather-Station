void httpUpdate()
{
  ...
  if ((minute(now()) % HTTP_UPDATE_INTERVAL == 0) || (minute(now()) == 0))
  {
    mlog(S_DEBUG, "HTTP update will happen later");
    return;
  }

  mlog(S_INFO, "HTTP update in progress");
  ...
  switch (ret)
  {
    case HTTP_UPDATE_FAILED:
      {
        mlog(S_ERROR, "HTTP_UPDATE_FAILED Error (" + String(ESPhttpUpdate.getLastError()) + "): " + ESPhttpUpdate.getLastErrorString());
        break;
      }
